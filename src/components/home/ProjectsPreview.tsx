...
const PROJECT_KEYS = ["gearbox", "enclosure"] as const;
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          {PROJECT_KEYS.map((key, i) => (
            <motion.div key={key} {...fadeIn(0.07 * i)} className="bg-surface border border-iris-dusk/40 rounded-sm p-6 hover:border-iris-dusk/50 transition-colors duration-200">
              ...
              <p className="font-display font-bold text-[16px] leading-[1.25] text-cold-pearl mb-4">{t(`${key}.title`)}</p>
              <p className="font-sans text-[13px] leading-[1.6] text-lavender-smoke mb-2">{t(`${key}.task`)}</p>
              <p className="font-technical text-[12px] tracking-[0.04em] text-cold-pearl">{t(`${key}.result`)}</p>
            </motion.div>
          ))}
        </div>
      </div>
